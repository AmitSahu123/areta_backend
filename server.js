...
const app = express();
app.use(cors({
  origin: 'https://areta360-web-5fex.vercel.app',
...
app.post('/vton', upload.fields([
  ...
  try {
    ...
    if (!req.files || !req.files['person_image'] || !req.files['person_image'][0]) {
      return res.status(400).send('Missing person_image file.');
    }
    if (!req.files['garment_image'] || !req.files['garment_image'][0]) {
      return res.status(400).send('Missing garment_image file.');
    }
